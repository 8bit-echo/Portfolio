...
var canvasWidth = windowWidth * 0.83333;
...
function SketchLine(numberOfVertices, easeFactor, speedFactor, colors, p, art) {
    ...
    for (var i = 0; i < numberOfVertices; i++) {
        curveVertices[i] = p.createVector(p.map(globalX,-450,370,0,p.canvas.width+50), p.map(globalY, 550,30,0,p.canvas.height+20));
    ...
    this.update = function() {
        ...
        for (var i = 0; i < numberOfVertices; i++) {
            distances[i].x = (i === 0) ? p.map(globalX,-450,370,0,p.canvas.width) - curveVertices[0].x : curveVertices[i - 1].x - curveVertices[i].x;
            distances[i].y = (i === 0) ? p.map(globalY, 550,30,0,p.canvas.height) - curveVertices[0].y : curveVertices[i - 1].y - curveVertices[i].y;
...
var s = function(p) {
    ...
    p.setup = function() {
        ...
        canvas = p.createCanvas(1200, 700);
    ...
    p.draw = function() {
        ...
        $("#hands").html = ("hands: " + globalHands.length);
        $('#X').html("X: " + globalX);
        $('#Y').html("Y: " + globalY);
        $('#Z').html("Z: " + globalZ);
        p.background(0);
